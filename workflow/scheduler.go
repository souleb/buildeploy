...
func (s *SchedulerService) convertToGraph(workflow *app.Workflow) (*dag.Graph, error) {
	...
	// Put jobs in temp Map in order to be able to check wether they exist in graph
	// If a job has a dependency that is not in the graph, return an error
	var g dag.Graph
	jobsMap := make(map[string]app.Job)
	for _, job := range workflow.Jobs {
		jobsMap[job.Name] = job
		if needs := job.Needs; needs != nil {
			for _, n := range needs {
				if !g.HasVertex(jobsMap[n]) {
					return nil, fmt.Errorf("job %s", n)
				}
				g.AddEdge(jobsMap[n], job, 1)
			}
		}
		g.Add(job)
	}
